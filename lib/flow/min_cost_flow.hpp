...
// 参考 AtCoder Library
// == ACLからの変更点 ==
// 初期状態に負のコストの辺がある場合に対応
// Primal-Dual(Dijkstra) + Bellman-Ford
// その場合の時間計算量はO(FVlogE + VE)
template <class Cap, class Cost> struct mcf_graph {
public:
    mcf_graph() {}
    mcf_graph(int n) : _n(n), g(n), has_negative_edge_initially(false) {}
    ...
    vector<pair<Cap, Cost>> slope(int s, int t, Cap flow_limit) {
        ...
        auto init_dual_ref = [&]() -> void {
            for(int i = 0; i < _n; i++) {
                ...
                if(!update) break;
                if(i == _n - 1) {
                    cerr << "This Graph may have some negative cycles." << endl;
                    exit(1);
                }
            }
        };
        ...
        auto dual_ref = [&]() {
            fill(dist.begin(), dist.end(), numeric_limits<Cost>::max() / 4);
